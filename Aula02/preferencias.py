...
with open('preferencias.json', 'r') as file:
    data = json.load(file)
# Processar os dados
preferencias_alimentacao = [
    item['Preferência de Alimentação'] for item in data]
...
preferencia_alimentacao_comum = max(
    set(preferencias_alimentacao), key=preferencias_alimentacao.count)

# Calcular o total de solicitações especiais
solicitacoes_especiais_total = sum(
    [len(s.split(',')) if s else 0 for s in solicitacoes_especiais])

# Calcular a atividade de lazer mais popular
atividades_lazer_flat = [
    item for sublist in atividades_lazer for item in sublist]
atividade_lazer_popular = max(
    set(atividades_lazer_flat), key=atividades_lazer_flat.count)

print(
    f"Preferência de Alimentação Mais Comum: {preferencia_alimentacao_comum}")
